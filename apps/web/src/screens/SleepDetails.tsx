...
import { formatDuration, formatTime } from "@sleep-tracker/shared";
...
export function SleepDetails() {
	...
	const efficiency = session.quality;
	...
	return (
		<div className="flex flex-col h-screen bg-background text-foreground">
			<div className="flex items-center justify-between px-6 py-4">
				<button 
					onClick={() => navigate(-1)}
					className="flex items-center justify-center size-11 rounded-full bg-card"
				>
			...
			<div className="flex-1 px-6 pb-24 overflow-y-auto">
				<div className="bg-card rounded-3xl p-6 mt-4">
					<div className="flex items-center justify-between mb-6">
						<div>
							<h2 className="text-2xl font-semibold font-heading">{formatDuration(session.duration)}</h2>
						...
						<div className="flex flex-col items-end">
							<div className="flex items-center gap-1">
								<div className="text-3xl font-semibold font-heading text-accent">{session.quality}</div>
					...
					<div className="flex items-center justify-between py-4 border-t border-border">
						...
						<span className="text-base font-semibold">{formatTime(session.startTime)}</span>
					</div>
					<div className="flex items-center justify-between py-4 border-t border-border">
						<div className="flex items-center gap-3">
						...
						<span className="text-base font-semibold">{formatTime(session.endTime)}</span>
					</div>
					<div className="flex items-center justify-between py-4 border-t border-border">
						<div className="flex items-center gap-3">
						...
						<span className="text-base font-semibold text-accent">{efficiency}%</span>
					</div>
				</div>
				<div className="mt-6">
						...
						<div className="flex items-start gap-3">
							...
							<div>
								...
								<p className="text-sm text-muted-foreground leading-relaxed">
									...
									efficiency of {efficiency}% shows you are spending most of your time in bed actually
									sleeping.
								</p>
							</div>
						</div>
					...
					<div className="bg-card rounded-2xl p-5 mt-4 border border-border">
						<div className="flex items-start gap-3">
							...
							<div>
								...
								<p className="text-sm text-muted-foreground leading-relaxed">
									Try to maintain consistent sleep and wake times. Going to bed around {formatTime(session.startTime)} has
